...
func TestBug03CorrectedMemberLeavesDirectionConflictState(t *testing.T) {
	...
	if err != nil || !first.DirectionConflict {
		t.Fatalf("expected initial direction conflict: %+v, %v", first, err)
	}
	if _, err := app.Members.Update(members[2].ID, MemberAddInput{No: "M003", MemberType: "梁", EndX: 10}); err != nil {
		t.Fatal(err)
	}
	second, err := app.Joints.Check(joint.ID)
	if err != nil || second.DirectionConflict || !second.Closed {
		t.Fatalf("corrected node did not recover: %+v, %v", second, err)
	}
	restored, err := app.Members.Get(members[2].ID)
	if err != nil {
		t.Fatal(err)
	}
	if restored.Status == model.MemberDirectionConflict {
		t.Fatalf("corrected member retained conflict state: %s", restored.Status)
	}
}
